...
4. **The lower force limit is too high.** The measurement has Helmholtz motion down to 0.2–0.3 N at β ≈ 0.1–0.2; the model's band for the G string at β = 0.1, v_b = 0.1 m/s is 1.3–3.0 N (measured 0.31–1.89 N). That band counts only forces where notes settle within 0.15 s; counting slow settling it is 0.9–3.0 N.
...
28. **Quiet attacks on the open G are the least reliable.** Over 24 wander seeds (`notes_stay_helmholtz_across_wander_seeds`, ignored by default) 2 of 24 pp attacks on G2 miss 0.15 s and one never settles out of multi-slip; the whole range has 4 failed checks in 720 notes. Near the lower edge the string is bistable, and the force band is fitted only to prompt settling from a steady bow, not to the performer's own attack (PLAN.md "Force band from prompt settling").
