...
49. **Firmer, slower attacks break high positions.** Tuned by ear in the plugin (September 2026): a slower soft attack (0.3 s), more bite (0.6 over 0.19 s), pressure tilt 0.3 and livelier wander (speed 0.2, β 0.052, 0.5 s). With them the seed sweep fails 37 checks where it allows 11 (3 with the current values): sul G D4 at pp–mf often never settles or plays 45–70 cents sharp, and a few open strings at pp miss their pitch. Mostly the pressure tilt (19 failures without it), which with the bite starts pp attacks above the Helmholtz band. Not applied. The range checks' fixed 150 ms settling limit would also need to follow the attack's length. The first mono renders sounded good; the stereo ones (`out/ab-stage/`) are not yet heard. Instances don't share their room and mics yet (Phase B): set them alike by hand.
