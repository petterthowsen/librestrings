...
1. **Soft, dark pp (item 40).** The bow's width darkened pp on the low strings above partial 8, but partials 4–7 are still about 13 dB too strong at pp, and the spectrum still doesn't brighten with the dynamics. Next: what else makes the model bright at pp (the pressure tilt at low dynamics, force, β). `compare --bridge` shows it is in the string and bow, not the body.
2. **Bow noise (item 42),** in the sustain and more at the attack: the target is the recording's HNR (26–34 dB) and the brighter, more obvious onset heard at mf. The bow's width made the model cleaner still (42–50 dB).
...
46. **Sections and the stage are not yet heard in a host** (docs/SECTIONS.md A2–A5): up to 12 players, each with its own seeded detune, lateness, vibrato, dynamics, bow position, pressure, timing and body; players 1–11 at 1× oversampling. They are placed on a stage (a near-coincident mic pair, 1/r, air absorption) in a shoebox room with first-order reflections, and the plugin plays them in stereo. Every spread in `Humanization`, the rooms, the absorption and the mic pair are first guesses; the humanization is in the tuning window ("Section") and was tuned by ear in Bitwig (September 2026).
49. **Firmer, slower attacks break high positions.** Tuned by ear in the plugin (September 2026): a slower soft attack (0.3 s), more bite (0.6 over 0.19 s), pressure tilt 0.3 and livelier wander (speed 0.2, β 0.052, 0.5 s). With them the seed sweep fails 37 checks where it allows 11 (3 with the current values then; 0 since the bow's width): sul G D4 at pp–mf often never settles or plays 45–70 cents sharp, and a few open strings at pp miss their pitch. Mostly the pressure tilt (19 failures without it), which with the bite starts pp attacks above the Helmholtz band. Not applied. The range checks' fixed 150 ms settling limit would also need to follow the attack's length. The first mono renders sounded good; the stereo ones (`out/ab-stage/`) are not yet heard. Instances don't share their room and mics yet (Phase B): set them alike by hand.
...
40. **The spectrum doesn't follow the dynamics.** Recorded notes brighten from pp to ff by 7–11 dB in partials 4–7 and 9–14 dB in partials 8–15; the model's spectrum stays about the same. The bow's width darkened pp on the low strings above partial 8 (C string −8 → −14 dB, recorded −32; centroid 4.7 → 3.0, recorded 1.3) and brought up the C string's pp fundamental (−10 → −5 dB, recorded −1), but partials 4–7 at pp are still about 13–15 dB too strong on the C and G strings, and at mf–ff the A string is now 13 dB too dark above partial 8 (−36 against −23).
