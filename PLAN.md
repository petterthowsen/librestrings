...
| **4. Realism pass** | Thermal friction, finger damping at note changes (a first, frequency-independent version is in: "Phase 3 notes: tuning and first listening"), the bow's width, bow noise, oversampling decision, bouncing-bow spiccato, sympathetic string coupling | A/B against recordings; clear improvement on attacks and legato transitions (the mdw attack data gives a measured target for attacks). **Started:** `strings-render compare` against the Iowa cello notes ("Phase 4: comparison with recorded notes"); the bow's width on the cello and bass ("The bow's width"); bow noise ("Bow noise"); thermal friction, opt-in ("Thermal friction") |
| **5. More instruments** ⚠️ | Violin, viola and double bass presets and bodies | Each instrument is convincing across its range. **All four built** (objective checks pass). The violin has been heard; the viola and bass not yet. See "Phase 5: the violin" and "Phase 5: viola and double bass" |
...
- **Where to find it:** `--thermal [SCALE]` on `bow`, `schelleng`, `measured`, `calibrate`, `play` and `compare`; the plugin's tuning window has a "Thermal friction" checkbox (Bow). A/B phrases in `out/ab-thermal/curve/` and `thermal/`.
