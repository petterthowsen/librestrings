...
| **5. More instruments** ⚠️ | Violin, viola and double bass presets and bodies | Each instrument is convincing across its range. **Violin built** (objective checks pass, not yet heard; see "Phase 5: the violin"); viola and double bass to come |
...
- **Performer:** the cello's mapping (β 0.115–0.07, the same timings). The violin strings have no flat zone (that needs torsion), so a wider β range is possible, but the band is only tested there. Per-instrument values live in `InstrumentSpec` (`beta`, `output_gain`, `seat`) and reach the performer through `PerformerSettings::for_instrument`; `PerformerSettings::default()` is the cello's. The output gain (0.5 against 0.065) matches the cello's median level on the example scales: the violin strings' impedance is about a third of the cello's.
- **Checks:** the range tests play G3–A6 and six high positions (12 and 19 semitones up the G, D and A strings) at pp, mf and ff: all Helmholtz within 150 ms and in tune (stopped notes within 5 cents with a steady bow; 10 with the wander). The seed sweep (`violin_notes_stay_helmholtz_across_wander_seeds`) has 3 failed checks over 1152 notes, all the open D at pp settling in 167–273 ms. E7 still plays Helmholtz and in tune at mf.
