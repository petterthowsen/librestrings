...
| **4. Realism pass** | Thermal friction, finger damping at note changes (a first, frequency-independent version is in: "Phase 3 notes: tuning and first listening"), the bow's width, bow noise, oversampling decision, bouncing-bow spiccato, sympathetic string coupling | A/B against recordings; clear improvement on attacks and legato transitions (the mdw attack data gives a measured target for attacks). **Started:** `strings-render compare` against the Iowa cello notes ("Phase 4: comparison with recorded notes"); the bow's width on the cello and bass ("The bow's width") |
