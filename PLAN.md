...
| **Polyphony** | Keyswitches: the white keys after the bow's (cello: G1 mono, A1 double stops, B1 divisi; violin and viola G2, A2, B2). Also a parameter | Mono, or double stops (4.3); divisi splits a section across a chord (5) |
| **Fingering** | — (parameter) | Near the nut & open, mid position, near the bridge: which strings play (4.3) |
| **Bow lift** | Keyswitches: white keys from the first C below the instrument's lowest note (cello: C1 off string, D1 on string; violin and viola: C2, D2; C4 = middle C). Also a parameter | What the bow does at the end of a note (4.3) |
...
- Every instance on the computer shares one stage (a memory-mapped file, as SWAM's instances share theirs): the editor's stage view shows every section as a box and moves any of them, and the room and mics are the same for all (docs/SECTIONS.md Phase B).
...
**Divisi** splits a section across the notes of a chord: each player takes one note, as the desks of a real section divide, instead of every player playing the whole chord (a double stop each). A player keeps the note it is playing while the chord holds it, so a note that comes in takes its players from the notes that are above their share of the section, and each of them ends that note and starts the new one; a note let go takes its players with it, and they rest until the next chord, so no player ever joins a note already sounding. Notes that arrive in the same sample (a MIDI chord) are divided once, before anyone starts; notes a few ms apart (a live-played chord) take their players when they arrive. The section divides only where it can: with one player it plays the whole chord, and a solo's divisi is the double stops it can play; with more notes than players every player still plays one note, and a note that comes in with no player free takes the one whose note is closest to it (of C–E–G, the C an octave above takes the G). In `strings-dsp` it is `Polyphony::Divisi` and `Section`'s chord (docs/SECTIONS.md A2, PLAN.md "Divisi").
...
| **6. Sections** ⚠️ | N-player engine, humanization, stage placement, divisi, SIMD across players | 12-player section within the CPU budget; sounds like a section, not a chorus effect. **Built** (docs/SECTIONS.md Phase A): up to 12 humanized players on a stage with early reflections, in the plugin, and divisi ("Divisi"); 12 players at 27.7% of real time without SIMD (over the 25% budget). The stage view and instance sync (docs/SECTIONS.md Phase B) are built: every instance's section on one stage, moved from any of them, one shared room. Not yet heard in a host |
...
- **Keyswitches:** the white keys from the first C below the lowest note (cello: C1 off string, D1 on string, E1 down-bow, F1 up-bow, G1 mono, A1 double stops, B1 divisi; they chose sustain, staccato and spiccato until "Phase 3 notes: playing like SWAM"; the polyphony keyswitches came later — the block moved down an octave on the violin, whose lowest note is only seven semitones above C3).
...
- Instrument and ensemble selection (only cello and solo are enabled), bow-lift and polyphony buttons that follow keyswitches, and fingering.
...
- **Plugin:** keyswitches sit at the highest C that keeps all seven white keys below the instrument's lowest note: violin and viola C2, cello C1, bass C0. The violin and viola therefore share C2 (the violin's keyswitches moved down an octave when the polyphony keyswitches joined: its lowest note is only seven semitones above C3, so C, D, E, F, G, A, B had to fit under C3). The Instrument box lists the instruments high to low. The telemetry used to store the instrument's enum value and read it as a position in that list; it now stores the position.
...
### Divisi (September 2026)

`Polyphony::Divisi` (docs/SECTIONS.md A2, §5): the notes of a chord are divided among the players, one each, as the desks of a section divide; until now every player played the whole chord (a double stop each). It changes only how the section's notes are routed, not the physics or the performer: a player of a divisi section still plays `Polyphony::DoubleStops`, which is what it takes when the section can't divide.

- **One division per sample.** `Section` keeps the note-ons that arrive in a sample in a fixed buffer and divides them at the start of `process` — and before any note-off, sustain or bow event, so the order a legato line needs is kept. A MIDI chord (every note at one timestamp) is therefore divided before anyone starts, and no player sounds a note it doesn't keep. The plugin handles events per sample, so its chords land in one sample. A human-played chord (a few ms apart) divides across those samples: the players a new note takes end the note they were playing (or drop the stroke still waiting for their humanizing delay, `Queue::remove`, rather than play it for those few ms) and start the new one.
- **The seating** (`Section::divide`): players keep the note they have while the chord holds it; the back of a note's players (the highest indices) moves to a new note first; the free players take the notes that just came in, lower-numbered players first. A note never gains players while it sounds — a note let go takes its players off, and they come back in with the next chord — so a passage of chord changes thins like a real section's rests. Shares are `players / notes`, the earlier notes one more where it doesn't divide: 8 players on 3 notes are 3/3/2, and 12 on 4 are 3/3/3/3. Front-to-back rather than interleaved alternate stands is a choice the ear hasn't judged yet.
- **Where it can't divide** — one player — every player plays the whole chord, sample for sample as `Polyphony::DoubleStops` does, and a solo's divisi is its double stops; the renderer's `--players 1` renders of a chord with `--divisi` and `--double-stops` are byte-identical.
- **More notes than players** (`Section::steal`): every player still plays one note, never a double stop. The players keep the notes they have, and a note that comes in with no player free takes the one whose note is closest to it in pitch (the earlier player where two are equally close), which ends that note; the note it leaves is dropped, because no player joins a note already sounding. Of C–E–G, a C an octave above takes the G player (`divisi_with_more_notes_than_players_steals_the_closest`).
- **The editor's keyboard** lights every note the section sounds, not only player 0's: `Section::sounding_notes` fills a bit per MIDI note into `Telemetry::sounding`, so a divided chord presses all its keys.
- **Cost** is routing only: a chord as divisi is *thinner* than the same chord as double stops (each player bows one string rather than two) — a two-chord score at 8 players, 14.8% of real time against 22.4%, about 3 dB quieter overall with each note louder (4 players bowing one note each, against 8 splitting the bow across two). Tests: the `divisi_*` tests in `tests/section.rs`, the divisi part of `a_section_never_allocates`, and `divisi_divides_a_chord_in_the_engine` in the plugin.
- **Not heard yet.** Renders (`out/divisi/`): the `divisi` score (a three-note chord, a moving line, two staccato chords, a unison) at 1, 4 (on the stage) and 8 players; and a two-chord score at 1 and 8 players with `--divisi` against `--double-stops` (its `poly` line taken out for the comparison; STATUS item 49).
