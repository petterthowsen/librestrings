...
- **Violin, viola and double bass arco (2012), in use:** the same project recorded them the same way, and `scripts/fetch-reference-data.sh iowa-violin | iowa-viola | iowa-bass` fetches those sets (68, 171 and 64 MB); `strings-render compare --instrument <name>` measures the model against each (PLAN.md "The other instruments' recorded notes"). The violin's and cello's files name the dynamic before the string (`Violin.arco.mf.sulG.G3B3.mono.wav`), the viola's and bass's after it (`Viola.arco.sulC.pp.C3B3.mono.wav`). The bass's set has both the extended C string (sulC, C1–Eb1) and the E string stopped at the extension's gates (sulE, E1 up). **The viola's 16/44.1 kHz files hold 96 kHz audio**: the take named `A4B4` reads 202 Hz at 44.1 kHz and 440.4 Hz read at 96 kHz; the site's individual stereo AIFFs of the same takes are the same, so the rate is wrong in the published files, not in the download or the AIFF→WAV conversion. `compare --file-rate 96000` reads the samples at 96 kHz and runs the model and the measurements there. The viola's takes are also much quieter at pp (−46 dBFS median against the cello's −36) and its notes swell over 0.4–0.5 s where the model's attacks are 10–30 ms.
...
- Good for: attack behavior. The same monochord setup as above. `strings-render guettler` compares the model with the waveforms stroke by stroke (PLAN.md "Attacks against measured data"). Notes: string A is D'Addario Prelude, B Helicore, C Thomastik Dominant and D Kaplan (Table 1 of the JASA-EL paper has T, μ, Z and EI; β = 5.5/70); `model_C_2` is the paper's "dominant 1". The file name's `Fb` and `a` are the measured mean bow force and acceleration. Each file starts about 40 ms before the stroke, and the bow accelerates from rest over the first 6 cm of a 12 cm stroke. The second record's archive is one 22 GB zip, but its MATLAB code (`HM_yn_gall.m`, `transient_analysis.m`, `detect_first_slip.m`) and per-session results (`matx.mat`: acceleration, force, transient) can be read out of it with HTTP range requests.
