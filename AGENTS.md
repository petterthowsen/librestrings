...
- The cello's static friction is μs 0.9, fitted to the measured attacks (`guettler`: at 0.8 the model needed twice the measured force for a given bow acceleration; PLAN.md "Attacks against measured data"); its quiet attacks are faster to match (`pp_attack` 0.6). `FrictionParams::default()` and the other instruments stay at 0.8.
